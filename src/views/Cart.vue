            ...
            <ul class="cart-item-list">
              <li v-for="item in cartList" :key="item.productImage">
                ...
                <div class="cart-tab-4">
                  <div class="item-price-total">{{parseInt(item.productNum * item.salePrice) | currency('￥')}}</div>
                </div>
                <div class="cart-tab-5">
                  <div class="cart-item-opration" @click="saveProductId(item.productId)">
        ...
        <div class="cart-foot-wrap">
          <div class="cart-foot-inner">
            ...
            <div class="cart-foot-r">
              ...
              <div class="btn-wrap">
                <a :class="{'btn--dis': checkedCount==0}" @click="checkOut" class="btn btn--red">Checkout</a>
              </div>
            </div>
          </div>
        </div>
...
import { editCheckAll, userCartList, deleteUserGood, userCartEdit } from "api/users";
export default {
  name: "Cart",
  data() {
    return {
  ...
  computed:{
    ...
    totalPrice() {
      ...
      this.cartList.forEach((item) => {
        if (item.checked == '1') {
          money += parseInt(item.productNum * item.salePrice)
        }
      })
      return money
    }
